...
async fn basic_derive(conn: ConnectionAsync) {
    let mut product = Product::new("Widget".to_string(), 100);
    product.save(&conn).await.unwrap();

    let loaded = Product::get(&conn, product.id).await.unwrap();
...
async fn derive_with_custom_table(conn: ConnectionAsync) {
    ...
    let loaded = Item::get(&conn, "ABC123".to_string()).await.unwrap();
...
async fn derive_query(conn: ConnectionAsync) {
    let mut p1 = Product::new("Cheap".to_string(), 50);
    let mut p2 = Product::new("Expensive".to_string(), 200);
    p1.save(&conn).await.unwrap();
    p2.save(&conn).await.unwrap();

    let results = query!(Product, price < 100).load(&conn).await.unwrap();
