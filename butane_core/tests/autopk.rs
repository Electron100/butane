...
async fn backend_autopk_sql_generation(conn: ConnectionAsync) {
    ...
    // Verify the SQL is appropriate for the backend
    if backend_name == "turso" {
        // Turso should NOT have STRICT
        assert!(
            !create_sql.contains("STRICT"),
            "Turso backend should not generate STRICT tables: {create_sql}"
        );
    } else if backend_name == "libsql" || backend_name == "sqlite" {
        // Libsql and SQLite should have STRICT
        assert!(
            create_sql.contains("STRICT"),
            "Libsql/SQLite backend should generate STRICT tables: {create_sql}"
        );
    }

    // Verify it has PRIMARY KEY (which auto-increments for INTEGER type in SQLite)
    // Note: AUTOINCREMENT keyword is only added for non-PK auto columns
    // For INTEGER PRIMARY KEY, auto-increment is automatic
    assert!(
