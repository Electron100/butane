...
fn debug_constraint_names(mut connection: Connection) {
    ...
    let drop_result2 = connection.execute("ALTER TABLE \"Order\" DROP CONSTRAINT \"Order_user_fkey\";");
    match drop_result2 {
        Ok(_) => println!("Successfully dropped \"Order_user_fkey\""),
        Err(e) => println!("Failed to drop \"Order_user_fkey\": {}", e),
    }
    ...
    let recreate_result = connection.execute("ALTER TABLE \"Order\" ADD FOREIGN KEY (\"user\") REFERENCES \"User\"(\"id\");");
